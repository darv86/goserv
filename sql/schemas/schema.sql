--
-- PostgreSQL database dump
--

-- Dumped from database version 16.2
-- Dumped by pg_dump version 16.2

SET statement_timeout = 0;
SET lock_timeout = 0;
SET idle_in_transaction_session_timeout = 0;
SET client_encoding = 'UTF8';
SET standard_conforming_strings = on;
SELECT pg_catalog.set_config('search_path', '', false);
SET check_function_bodies = false;
SET xmloption = content;
SET client_min_messages = warning;
SET row_security = off;

SET default_tablespace = '';

SET default_table_access_method = heap;

--
-- Name: feeds; Type: TABLE; Schema: public; Owner: darv
--

CREATE TABLE public.feeds (
    id bigint NOT NULL,
    created_at timestamp without time zone DEFAULT now() NOT NULL,
    updated_at timestamp without time zone DEFAULT now() NOT NULL,
    name text NOT NULL,
    url text NOT NULL,
    user_id bigint NOT NULL,
    fetched_at timestamp without time zone
);


ALTER TABLE public.feeds OWNER TO darv;

--
-- Name: feeds_id_seq; Type: SEQUENCE; Schema: public; Owner: darv
--

ALTER TABLE public.feeds ALTER COLUMN id ADD GENERATED ALWAYS AS IDENTITY (
    SEQUENCE NAME public.feeds_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
    CACHE 1
);


--
-- Name: goose_db_version; Type: TABLE; Schema: public; Owner: darv
--

CREATE TABLE public.goose_db_version (
    id integer NOT NULL,
    version_id bigint NOT NULL,
    is_applied boolean NOT NULL,
    tstamp timestamp without time zone DEFAULT now() NOT NULL
);


ALTER TABLE public.goose_db_version OWNER TO darv;

--
-- Name: goose_db_version_id_seq; Type: SEQUENCE; Schema: public; Owner: darv
--

ALTER TABLE public.goose_db_version ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME public.goose_db_version_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
    CACHE 1
);


--
-- Name: posts; Type: TABLE; Schema: public; Owner: darv
--

CREATE TABLE public.posts (
    id bigint NOT NULL,
    title text NOT NULL,
    url text NOT NULL,
    feed_id bigint NOT NULL
);


ALTER TABLE public.posts OWNER TO darv;

--
-- Name: posts_id_seq; Type: SEQUENCE; Schema: public; Owner: darv
--

ALTER TABLE public.posts ALTER COLUMN id ADD GENERATED ALWAYS AS IDENTITY (
    SEQUENCE NAME public.posts_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
    CACHE 1
);


--
-- Name: users; Type: TABLE; Schema: public; Owner: darv
--

CREATE TABLE public.users (
    id bigint NOT NULL,
    created_at timestamp without time zone DEFAULT now() NOT NULL,
    updated_at timestamp without time zone DEFAULT now() NOT NULL,
    name text NOT NULL,
    api_key character varying(64) DEFAULT encode(sha256(((random())::text)::bytea), 'hex'::text) NOT NULL
);


ALTER TABLE public.users OWNER TO darv;

--
-- Name: users_id_seq; Type: SEQUENCE; Schema: public; Owner: darv
--

ALTER TABLE public.users ALTER COLUMN id ADD GENERATED ALWAYS AS IDENTITY (
    SEQUENCE NAME public.users_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
    CACHE 1
);


--
-- Name: feeds feeds_url_key; Type: CONSTRAINT; Schema: public; Owner: darv
--

ALTER TABLE ONLY public.feeds
    ADD CONSTRAINT feeds_url_key UNIQUE (url);


--
-- Name: goose_db_version goose_db_version_pkey; Type: CONSTRAINT; Schema: public; Owner: darv
--

ALTER TABLE ONLY public.goose_db_version
    ADD CONSTRAINT goose_db_version_pkey PRIMARY KEY (id);


--
-- Name: feeds pkFeeds; Type: CONSTRAINT; Schema: public; Owner: darv
--

ALTER TABLE ONLY public.feeds
    ADD CONSTRAINT "pkFeeds" PRIMARY KEY (id);


--
-- Name: posts pkPosts; Type: CONSTRAINT; Schema: public; Owner: darv
--

ALTER TABLE ONLY public.posts
    ADD CONSTRAINT "pkPosts" PRIMARY KEY (id);


--
-- Name: users pkUsers; Type: CONSTRAINT; Schema: public; Owner: darv
--

ALTER TABLE ONLY public.users
    ADD CONSTRAINT "pkUsers" PRIMARY KEY (id);


--
-- Name: posts posts_url_key; Type: CONSTRAINT; Schema: public; Owner: darv
--

ALTER TABLE ONLY public.posts
    ADD CONSTRAINT posts_url_key UNIQUE (url);


--
-- Name: users users_api_key_key; Type: CONSTRAINT; Schema: public; Owner: darv
--

ALTER TABLE ONLY public.users
    ADD CONSTRAINT users_api_key_key UNIQUE (api_key);


--
-- Name: akFeeds; Type: INDEX; Schema: public; Owner: darv
--

CREATE UNIQUE INDEX "akFeeds" ON public.feeds USING btree (name);


--
-- Name: akPosts; Type: INDEX; Schema: public; Owner: darv
--

CREATE UNIQUE INDEX "akPosts" ON public.posts USING btree (url);


--
-- Name: akUsers; Type: INDEX; Schema: public; Owner: darv
--

CREATE UNIQUE INDEX "akUsers" ON public.users USING btree (name);


--
-- Name: feeds feeds_user_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: darv
--

ALTER TABLE ONLY public.feeds
    ADD CONSTRAINT feeds_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.users(id) ON DELETE CASCADE;


--
-- Name: posts posts_feed_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: darv
--

ALTER TABLE ONLY public.posts
    ADD CONSTRAINT posts_feed_id_fkey FOREIGN KEY (feed_id) REFERENCES public.feeds(id) ON DELETE CASCADE;


--
-- PostgreSQL database dump complete
--

